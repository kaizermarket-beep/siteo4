...
function GalleryScene({
  ...
  const materials = useMemo(
    () => Array.from({ length: visibleCount ?? 8 }, () => createClothMaterial()),
    [visibleCount]
  );

  const spatialPositions = useMemo(() => {
    const positions: { x: number; y: number }[] = [];
    const count = visibleCount ?? 8;
  ...
  useEffect(() => {
    planesData.current = Array.from({ length: count }, (_, i) => ({
      index: i,
      z: count > 0 ? ((depthRange / Math.max(count, 1)) * i) % depthRange : 0,
      imageIndex: totalImages > 0 ? i % totalImages : 0,
      x: spatialPositions[i]?.x ?? 0,
      y: spatialPositions[i]?.y ?? 0,
    }));
  }, [depthRange, spatialPositions, totalImages, count]);
...
function FallbackGallery({ images }: { images: ImageItem[] }) {
  ...
  return (
    <div className="flex h-full flex-col items-center justify-center bg-neutral-900 p-4">
      <p className="mb-4 text-neutral-400">Galerie (mode simplifié) :</p>
